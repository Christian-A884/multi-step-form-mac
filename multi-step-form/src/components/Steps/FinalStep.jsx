...
const FinalStep = () => {
	const { userPlan, period, addonOptions } = useContext(SwitchContext);
			...
			<div className="flex flex-col bg-very_light_grey px-[24px] rounded-lg py-4">
				<div className="flex justify-between items-center border-b border-light_grey">
					<div className="flex flex-col justify-start items-start">
						<p className="text-sm text-denim">{userPlan.title} {period === 'monthly' ? '(Monthly)' : '(Yearly)'}</p>
						<button className="text-sm text-grey underline pb-2 leading-5 ">Change</button>
					</div>
					<div>
						<span
						className="text-sm text-denim font-bold ">{period === 'monthly' ? userPlan.price : userPlan.priceYear}</span>
					</div>
				</div>
